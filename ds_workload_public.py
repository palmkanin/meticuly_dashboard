...
# --- Read full_log.xlsx
url_full_log = 'https://github.com/palmkanin/meticuly_dashboard/blob/main/em_designer.xlsx'
myfile_full_log = requests.get(url_full_log)

full_log = pd.read_excel(myfile_full_log.content, index_col=0)

# --- Read userID
url_userID = 'https://github.com/palmkanin/meticuly_dashboard/blob/main/userID.xlsx'
myfile_userID = requests.get(url_userID)
userID = pd.read_excel(myfile_userID.content, index_col=0)

# --- Read em_designer
url_em_designer = 'https://github.com/palmkanin/meticuly_dashboard/blob/main/em_designer.xlsx'
...
em_designer = em_designer.dropna(subset= 1)
...
# group_current_log['start'] = pd.to_timedelta(group_current_log['start'])
# group_current_log['session_duration'] = group_current_log['today'] - group_current_log['start']
...
for i in range(len(ds_list)):
    ...
    with ds_tabs[i]:
        def get_o_n(full_log_merge, start_date, end_date):
            ...
            ## -- Business Day
            bday = pd.bdate_range(start_date, end_date)
            bday_count = len(bday)
